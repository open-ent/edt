    ...
    ['$scope', '$location','$routeParams',  ($scope, $location, $routeParams) => {
        ...
        $scope.saveCourse = async (course: Course): Promise<void>  => {
            ...
            if($scope.editOccurrence === true){
                course.syncCourseWithOccurrence($scope.courseOccurrenceForm, $scope.display, $scope.courseOccurrences);
        ...
        $scope.isPastDate = () => {
            if ($scope.course.timeSlot.start && !$scope.display.freeSchedule) {
                return (moment($scope.course.startDate)
                    .add(moment($scope.course.timeSlot).minutes(),'minutes')
                    .add(moment($scope.course.timeSlot.start).hours(),'hours')
