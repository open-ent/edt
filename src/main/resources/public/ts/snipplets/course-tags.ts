...
const vm: IViewModel = {
    ...
    getCourseTags: async (): Promise<void> => {
        vm.courseTags = await courseTagService.getCourseTags(window.model.vieScolaire.structure.id);
        that.safeApply();
    },


    createCourseTag: async (): Promise<void> => {
        courseTagService.createCourseTag(window.model.vieScolaire.structure.id, vm.courseTagForm)
            ...
            }).catch((_: AxiosError): void => toasts.warning("edt.admin.course.tags.form.create.error"));
    },

    deleteCourseTag: async (courseTagId: number): Promise<void> => {
        courseTagService.deleteCourseTag(window.model.vieScolaire.structure.id, courseTagId)
            .then((res: AxiosResponse): void => {
            ...
            }).catch((_: AxiosError): void => toasts.warning("edt.admin.course.tags.form.delete.error"));
    },

    updateCourseTag: async (): Promise<void> => {
        courseTagService.updateCourseTag(vm.courseEditTagForm)
            .then((res: AxiosResponse): void => {
                if (res.status === 200 || res.status === 201) {
            ...
            }).catch((_: AxiosError): void => toasts.warning("edt.admin.course.tags.form.edit.error"));
    },
    ...
    toggleVisibility: async (courseTag: CourseTag): Promise<void> => {
        await courseTagService.updateCourseTagHidden(window.model.vieScolaire.structure.id, courseTag.id, !courseTag.isHidden);
        vm.getCourseTags();
    },
